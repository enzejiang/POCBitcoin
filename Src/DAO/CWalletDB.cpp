...
{
    WalletServ::getInstance()->AddMapAddress(strAddress, strName);
...
{
    WalletServ::getInstance()->EraseAddress(strAddress);
    string key = string("name")+strAddress;
...
bool CWalletDB::LoadWallet(vector<unsigned char>& vchDefaultKeyRet)
{
    ...
    int64 nTransactionFee = 0;//WalletServ::getInstance()->nTransactionFee;
    ...
    loop
    {
        ...
        if (0 == strType.find_first_of("name"))
        {
            ...
            WalletServ::getInstance()->AddMapAddress(strAddress, name);
            ...
            if (bRet)
            {
                ...
                if(UnSeriaWalletTx(cProtoc, wtx))
                {
                    ...
                    if (pos != string::npos)
                    {
                        ...
                        if (wtx.GetHash() != hash) {
                            printf("%s---%d\n", __FILE__, __LINE__);
                            printf("hash mismatch readHash[%s]  CalcHash[%s]\n", hash.ToString().c_str(), wtx.GetHash().ToString().c_str());
                            continue;
                        }
                       
                        WalletServ::getInstance()->AddToWallet(hash, wtx);
        ...
        else if (0 == strType.find_first_of("key"))
        {
           ...
           // ssKey >> vchPubKey;
            ...
            WalletServ::getInstance()->AddPrivKey(vchPubKey, vchPrivKey);
            WalletServ::getInstance()->AddPubKey(Hash160(vchPubKey), vchPubKey);
        }
        ...
        else if (0 == strType.find_first_of("setting"))  /// or settings or option or options or config?
        {
            ...
            else if (string::npos != strType.find_first_of("nTransactionFee"))
            {
                ...
                WalletServ::getInstance()->setFee(nTransactionFee);
#if 0
